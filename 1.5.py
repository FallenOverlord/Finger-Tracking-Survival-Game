...
while running:
    ...
    if elapsed_time <= 10:
        ...
        chaser_color = BLOOD
    # Increase the chance of generating chasers based on elapsed time
    if random.random() < 0.005 * (elapsed_time / 30 + 1):  # Starts at 0.5% and increases
        new_chaser = {
...
for chaser in chasers[:]:  # Loop through a copy of the chasers list
    # Move chaser towards player
    if chaser["x"] < player_x:
        chaser["x"] += chaser["speed"]
    elif chaser["x"] > player_x:
        chaser["x"] -= chaser["speed"]

    if chaser["y"] < player_y:
        chaser["y"] += chaser["speed"]
    elif chaser["y"] > player_y:
        chaser["y"] -= chaser["speed"]

    # Draw the chaser
    pygame.draw.circle(screen, RED, (int(chaser["x"]), int(chaser["y"])), chaser["radius"])
    ...
    for shape in shapes[:]:
        ...
        # Check for collision between chaser and shapes
        for shape in shapes:
            dist_to_shape = ((chaser["x"] - (shape[0] + shape[2] / 2))**2 + (chaser["y"] - (shape[1] + shape[2] / 2))**2)**0.5
            if dist_to_shape < (shape[2] / 2 + chaser["radius"]):
                chasers.remove(chaser)
                break  # A chaser can only collide with one shape at a time

        # Check for collision between chasers
        for other_chaser in chasers:
            if chaser != other_chaser:
                dist_to_chaser = ((chaser["x"] - other_chaser["x"])**2 + (chaser["y"] - other_chaser["y"])**2)**0.5
                if dist_to_chaser < (chaser["radius"] + other_chaser["radius"]):
                    chasers.remove(chaser)
                    break  # A chaser can only collide with one other chaser at a time

        dist = ((chaser["x"] - (player_x + player_size / 2))**2 + (chaser["y"] - (player_y + player_size / 2))**2)**0.5
